...
export default function Navigator() {
  ...
  const checkImgURL = () => {
    ...
    if (regExpHttp.test(userInfo.profileImgUrl) || regExpHttps.test(userInfo.profileImgUrl)) return userInfo.profileImgUrl;
      ...
      <Navbar className="sticky z-50 top-0 border-b opacity-90" fluid={true} rounded={true}>
        ...
        <div className="flex md:order-2">
          {login ? (
            ...
            <>
              {userInfo ? (
                <Dropdown className="transition ease-in" arrowIcon={false} inline={true} label={<Avatar className="rounded-full border mr-3" alt="User settings" img={checkImgURL()} rounded={true} />}>
                  <Dropdown.Header>
            ...
            <>
              <Button onClick={() => setShow(true)} className="transition ease-in mr-3 bg-ezip-green hover:bg-ezip-green_hover">
                로그인
              </Button>
        ...
        <Navbar.Collapse className="mr-auto">
          <Navbar.Link className="transition ease-in font-suiteMedium text-base text-slate-400 hover:text-ezip-green_hover md:hover:text-ezip-green_hover" href="/discover">
            발견
          </Navbar.Link>
        </Navbar.Collapse>
        <div className="hidden md:block w-1/3 mr-5">
          <form className="flex items-center">
            ...
            <button className="transition ease-in p-2 ml-2 text-sm font-suiteMedium text-white bg-ezip-green rounded-lg hover:bg-ezip-green_hover" onClick={handleSearch}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                <path fillRule="evenodd" d="M10.5 3.75a6.75 6.75 0 100 13.5 6.75 6.75 0 000-13.5zM2.25 10.5a8.25 8.25 0 1114.59 5.28l4.69 4.69a.75.75 0 11-1.06 1.06l-4.69-4.69A8.25 8.25 0 012.25 10.5z" clipRule="evenodd" />
              </svg>
